...
public class InputPipeBlockEntity extends PipeBlockEntity{

    public InputPipeBlockEntity(BlockPos pos, BlockState state) {
        super(WONBlockEntities.INPUT_PIPE, pos, state);
    }
                ...
                if (storage != null){
                    try(Transaction tx = Transaction.openOuter()){
                        for (StorageView<FluidVariant> view : storage) {
                            ...
                            var extracted = view.extract(resource, Math.min(MAX_FLUID_PER_TICK, blockEntity.storage.getCapacity() - blockEntity.storage.amount), tx);
                            blockEntity.storage.insert(resource, extracted, tx);
                        }
                        tx.commit();
